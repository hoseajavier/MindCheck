...
export default function KuisionerPage() {
  ...
  const handleSelect = (index: number) => {
    ...
    // Auto-advance halus setelah memilih opsi jawaban
    if (current < total - 1) {
      setTimeout(() => {
  ...
  return (
    <div className="flex min-h-screen bg-[#F8FAFC]">
      ...
      {/* Ditambahkan pt-20 agar di layar mobile tidak tertutup oleh tombol hamburger sidebar */}
      <div className="flex-1 p-5 md:p-10 pt-20 md:pt-10 max-w-4xl mx-auto w-full flex flex-col justify-start">
        {/* HEADER */}
        <div className="mb-6 border-b border-gray-200 pb-4">
          <h1 className="text-2xl md:text-3xl font-extrabold text-[#2C3E50] tracking-tight">
        ...
        <div className="bg-white rounded-2xl p-4 shadow-2xs border border-gray-100 mb-6">
          <div className="flex justify-between items-center mb-2.5">
            <span className="text-xs font-bold uppercase tracking-wider text-gray-400">
              Progres Pengisian
            </span>
            <span className="text-sm font-bold text-[#6FA8A1] bg-[#E2F2F0] px-3 py-1 rounded-full">
              {current + 1} / {total} Pertanyaan
            </span>
          </div>
          <div className="w-full bg-slate-100 h-2.5 rounded-full overflow-hidden">
            <div
              className="bg-[#6FA8A1] h-full rounded-full transition-all duration-300 ease-out shadow-[0_0_8px_rgba(111,168,161,0.5)]"
        ...
        {/* MAIN QUESTION CARD */}
        <div className="bg-white rounded-3xl shadow-xs border border-gray-100/80 p-6 md:p-8 flex-1 flex flex-col justify-between min-h-[400px]">
          {/* QUESTION TEXT */}
          <div className="space-y-4">
            <div className="flex items-center gap-2 text-[#6FA8A1]">
              <HelpCircle className="w-5 h-5 shrink-0" />
              <span className="text-xs font-bold uppercase tracking-widest text-gray-400">
                Pertanyaan Ke-{current + 1}
              </span>
            </div>
            <h2 className="text-lg md:text-xl font-bold text-[#2C3E50] leading-relaxed">
              {questions[current]}
            </h2>
          </div>
          ...
          <div className="my-8 space-y-3">
            {options.map((opt, i) => {
              ...
              return (
                <label
                  key={i}
                  className={`flex items-center justify-between border rounded-2xl px-5 py-4 cursor-pointer transition-all duration-200 group
                    ${
                      isChecked
                        ? "bg-[#E2F2F0] border-[#6FA8A1] text-[#244b46] ring-1 ring-[#6FA8A1]"
                        : "bg-white border-gray-200/80 hover:border-gray-300 hover:bg-slate-50 text-gray-700"
                    }
                  `}
                >
                  <div className="flex items-center gap-3.5">
                    <div
                      ...
                      ${
                        isChecked
                          ? "border-[#6FA8A1] bg-[#6FA8A1] text-white"
                    ...
                    <span
                      className={`text-sm md:text-base ${isChecked ? "font-bold" : "font-medium"}`}
                    >
                      {opt}
                    </span>
                  </div>
          ...
          <div className="flex justify-between items-center border-t border-gray-100 pt-5 mt-auto">
              ...
              className={`flex items-center gap-1.5 px-5 py-2.5 rounded-xl font-semibold text-sm transition-all
                ${
                  current === 0
                    ...
                    : "bg-slate-100 hover:bg-slate-200 text-gray-700 cursor-pointer active:scale-95"
                }
              `}
            >
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-6 py-2.5 text-white font-bold text-sm rounded-xl transition-all shadow-xs
                ${
                  answers[current] === undefined
                    ? "bg-[#6FA8A1]/40 cursor-not-allowed"
                    : "bg-[#6FA8A1] hover:bg-[#598F88] hover:shadow-md cursor-pointer active:scale-95"
                }
              `}
            >
              {current === total - 1 ? (
